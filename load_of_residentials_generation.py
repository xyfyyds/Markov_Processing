...
residential_list = [residential_1, residential_2, residential_3, residential_4, residential_5, residential_6]

for i, res in enumerate(residential_list):
    selected_data = df[res]
    output_csv_file = './data_generated/residential_power/power_of_residential'+ '_' + str(i+1) +'.csv'
...
for f in file_path:
    ...
    diff_data.to_csv(f[:-4] + '_changes.csv', index=False)
